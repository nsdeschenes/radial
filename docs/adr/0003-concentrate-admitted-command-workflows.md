...
The command owns application configuration, request assembly, operational telemetry, interruption and expected-failure mapping, output, and disposal of the CLI application scope. Command modules cannot inspect raw invocation arguments or select runtime command identity; one frozen environment snapshot supplies telemetry and runtime configuration. Data Status enters through that application scope and a shared-runtime lease while remaining lazy, read-only, and non-creating.

## Considered Options

Framework-neutral inner handlers behind caller-owned wrappers were rejected because they split one command workflow across modules and made tests target an interface the Public CLI did not call. A shared per-command orchestrator, raw arguments in command modules, and runtime command identity were rejected because each would leak command policy across the boundary. A direct Data Status database opener was rejected because it bypasses shared runtime ownership; the application scope instead preserves the read-only status contract through a lazy runtime lease. Dynamically importing a deep entry module was rejected because the module cannot place its own loading failure inside an admitted lifecycle; the entry modules instead remain eager and import-light while operational dependencies stay lazy.
