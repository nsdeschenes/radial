...
const CURRENT_PRIVATE_TABLE_DEFINITIONS = [
  ...
  "facility_variation_audits:CREATE TABLE radial_producer.facility_variation_audits(snapshot_id UUID, source_record_id VARCHAR, outcome VARCHAR NOT NULL, source_identity VARCHAR, CHECK((outcome IN ('matched', 'outside-source-coverage', 'no-unique-match', 'unusable-source-value'))), PRIMARY KEY(snapshot_id, source_record_id));",
  ...
  'navaid_snapshots:CREATE TABLE radial_producer.navaid_snapshots(snapshot_id UUID PRIMARY KEY, snapshot_checksum VARCHAR NOT NULL, raw_navaids_checksum VARCHAR NOT NULL, planner_navaids_checksum VARCHAR NOT NULL, exclusions_checksum VARCHAR NOT NULL, facility_variation_audits_checksum VARCHAR NOT NULL, retrieved_at TIMESTAMP WITH TIME ZONE NOT NULL, retrieval_completed_at TIMESTAMP WITH TIME ZONE NOT NULL, published_at TIMESTAMP WITH TIME ZONE NOT NULL, source_identity VARCHAR NOT NULL, derivation_policy_identity VARCHAR NOT NULL, matching_policy_identity VARCHAR NOT NULL, raw_navaid_count INTEGER NOT NULL, planner_navaid_count INTEGER NOT NULL, exclusion_count INTEGER NOT NULL, magnetic_model VARCHAR NOT NULL, magnetic_model_version VARCHAR NOT NULL, magnetic_model_epoch_year DOUBLE NOT NULL, magnetic_reference_date DATE NOT NULL, magnetic_model_source VARCHAR NOT NULL, magnetic_model_checksum VARCHAR NOT NULL, CHECK((raw_navaid_count >= 0)), CHECK((planner_navaid_count >= 0)), CHECK((exclusion_count >= 0)), CHECK(((planner_navaid_count + exclusion_count) = raw_navaid_count)));',
