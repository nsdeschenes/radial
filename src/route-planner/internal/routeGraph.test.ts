...
type SelectedRoute = NonNullable<ReturnType<typeof routeGraph.selectOptimalRoute>>;
...
test('selects the shorter Route Plan even when it contains more Route Legs', () => {
  ...
  const selected = routeGraph.selectOptimalRoute(
...
test('uses exact Float64 distance, then Route Leg count, then stable Navaid identity sequence', () => {
  ...
  expect(routeGraph.selectOptimalRoute([nextFloat, exact], [], 2)?.navaids).toEqual([
    exact.routePoint,
  ]);
  expect(
    routeGraph.selectOptimalRoute(
  ...
  expect(
    routeGraph.selectOptimalRoute([stableIdSecond, stableIdFirst], [], 20)?.navaids
  ).toEqual([stableIdFirst.routePoint]);
  expect(
    routeGraph.selectOptimalRoute([identifierSecond, identifierFirst], [], 20)?.navaids
...
test('produces deeply equal structured results for ten deterministic input permutations', () => {
  ...
  const expected = routeGraph.selectOptimalRoute(
    [first, second, direct],
...
test(`matches an exhaustive reference solver for 1,000 generated small graphs (seed ${PROPERTY_SEED})`, () => {
  ...
  for (let example = 0; example < 1_000; example += 1) {
    ...
    const actual = routeGraph.selectOptimalRoute(
      graph.candidates,
      graph.legs,
...
function isCounterexample(graph: GeneratedGraph): boolean {
  return !deeplyEqual(
    routeGraph.selectOptimalRoute(graph.candidates, graph.legs, graph.maximumDistanceNm),
